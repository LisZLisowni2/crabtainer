...
pub async fn from_image(base_image: &String, output_layout_name: &String) -> Result<(), String> {
    let files = std::fs::read_dir(RustockerPaths::image_store_dir())
    ...
    println!(
        " => [FROM] Extract rootfs from {} to {}'s rootfs",
        base_image, output_layout_name
    );
    let path = RustockerPaths::layout_store_dir()
